...
export default function Page() {
  ...
  async function send() {
    ...
    // If embedded from a different origin (Wix), use FULL URL:
    ...
    try {
      ...
      if (!resp.ok) {
        ...
        setMessages([...next, {
          role: "assistant",
          content: `Error ${resp.status}: ${detail || "request failed"}`
        }]);
        return;
      }
      ...
      setMessages([...next, {
        role: "assistant",
        content: `Network error: ${String(e?.message || e)}`
      }]);
    }
  }
      ...
      <div ref={boxRef} style={{ height: 420, overflowY: "auto", padding: 12, borderRadius: 12, background: "#111827" }}>
        {messages.map((m, i) => (
          <div key={i} style={{ margin: "12px 0" }}>
            <div style={{ opacity: 0.7, fontWeight: 600 }}>{m.role === "user" ? "You" : "Bot"}</div>
